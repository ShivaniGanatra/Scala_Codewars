object IsValidWalk extends App{
...
//you know it takes you one minute to traverse one city block, so create a function that will return
...
//    or late!) and will, of course, return you to your starting point. Return false otherwise.
  ...
  def isValidWalk(walk: Seq[Char]): Boolean = {
    if (walk.count(_ == 'e') == walk.count(_ == 'w') && walk.count(_ == 'n') == walk.count(_ == 's') && walk.length == 10) {
      return true
    }
    false
  }

  println(isValidWalk(Seq('n','s','n','s','n','s','n','s','n','s')))
